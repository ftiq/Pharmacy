...
patch(PosStore.prototype, {
    ...
    async getEditedPackLotLines(isAllowOnlyOneLot, packLotLinesToEdit, productName) {
        ...
        const foundLocation = Object.values(stock_location).find(loc => loc.id === lot_stock_id);
        if (foundLocation) {
            parentLocation = foundLocation.location_id[0];
        }

        const resstockLocations = Object.values(stock_location)
            .filter(loc => loc.parent_path?.includes(`1/${parentLocation}/${lot_stock_id}`))
            .map(loc => loc.id);

        const stock_quant_lot = Object.values(stock_quant)
            .filter(quant =>
                resstockLocations.includes(quant.location_id?.[0]) &&
                quant.product_id?.[0] === product.id
            )
            .map(quant => quant.lot_id?.[0]);

        const product_lot = list_lot_num.filter(
            lot => stock_quant_lot.includes(lot.id) && lot.total_qty > 0
        );

        if (!product_lot.length) {
            await this.env.services.popup.add(ErrorPopup, {
                title: _t("Stock Error"),
                body: _t(`No stock available for ${productName}`),
            });
            return;
        }
        ...
        if (payload.newArray.length && this.list_lot_num.some(
            lot => lot.name === payload.newArray[0].text && lot.total_qty <= 0
        )) {
            await this.env.services.popup.add(ErrorPopup, {
                title: _t("Stock Error"),
                body: _t("Selected lot has no available quantity."),
            });
            return;
        }
            ...
            if (this.config.sh_display_by === "warehouse") {
                for (let stock of stocks) {
                    if (stock.warehouse_id in warehouse_by_id) {
                        ...
                        warehouse_by_id[stock.warehouse_id] = {
